...
class RobotEnv(gym.Env):
    ...
    def reset(self, seed=None, options=None):
        ...
        flattened_state = np.concatenate([
            np.array([normalized_angle], dtype=np.float32),
    ...
    def check_robot_state(self, current_center, angle):
        ...
        # Remove boundary violation check - we'll let the robot continue even near boundaries
    ...
    def step(self, action):
        ...
        # Get normalized_angle at the very start
        initial_normalized_angle = angle / 360.0 if angle else 0.0
        initial_normalized_angle = float(f"{initial_normalized_angle:.3f}")
        
        # Continue with state check but use initial_normalized_angle
        _, done, truncated, info = self.check_robot_state(current_center, angle)
        ...
        flattened_state = np.concatenate([
            np.array([initial_normalized_angle], dtype=np.float32),
            np.array(prev_phase_values, dtype=np.float32),  # Include the captured phase values
            np.array(prev_amplitude_values, dtype=np.float32)  # Include the captured amplitude values
        ])
